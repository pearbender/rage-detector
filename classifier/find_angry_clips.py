...
def plot_audio_segment(segment):
    ...
    # Create a time array
    times = np.arange(len(normalized_samples)) / segment.frame_rate

    # Plot the audio segment
    plt.figure(figsize=(12, 4))
    plt.plot(times, normalized_samples, color='b')
...
def main():
    ...
    for i, file_path in enumerate(file_paths):
        ...
        nonsilent_sections = silence.detect_nonsilent(audio, min_silence_len, -45, 100)
        ...
        for [nonsilent_start, nonsilent_end] in nonsilent_sections:
            while nonsilent_end - nonsilent_start > maximum_section_length:
                new_nonsilent_sections.append([nonsilent_start, nonsilent_start + maximum_section_length])
        ...
        for [nonsilent_start, nonsilent_end] in nonsilent_sections:
            print(f"{file_path} ({i + 1}/{len(file_paths)}): {round(100 * nonsilent_start / len(audio))}%", end='\r')
            ...
            text_segments, _ = whisper_model.transcribe(temp_file, **whisper_options)
            ...
            clip_end = min(len(audio), nonsilent_start + peak_amplitude_offset + 1000)
            clipped_audio_segment = audio[clip_start:clip_end]
            clipped_audio_segment.export(f"data/angry/{base_name_without_extension}_{clip_start}.wav", 'wav')
            angry_clip_count += 1

        random_sections = random.sample(nonsilent_sections, angry_clip_count)
        for [nonsilent_start, nonsilent_end] in random_sections:
            audio_segment = audio[nonsilent_start:nonsilent_end]
            audio_segment.export(f"data/not-angry/{base_name_without_extension}_{nonsilent_start}.wav")
        print('')
